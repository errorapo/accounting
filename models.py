...
class Sales(db.Model):
    ...
    supply_type = db.Column(db.String(10), default='intra')
    total_amount = db.Column(db.Numeric(precision=15, scale=2))
    payment_type = db.Column(db.String(20), default='cash')
    payment_status = db.Column(db.String(20), default='paid')
    invoice_date = db.Column(db.Date, default=lambda: datetime.now(timezone.utc).date)
    created_at = db.Column(db.DateTime, default=utc_now)
    created_by = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)
...
class CostCenter(db.Model):
    """Cost Center for tracking P&L by department/project."""
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), unique=True, nullable=False)
...
class AuditLog(db.Model):
    """Audit trail - tracks all changes to records."""
    id = db.Column(db.Integer, primary_key=True)
