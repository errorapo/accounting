...
def record_sale(date, customer_name, amount, payment_type='cash', description="Sale", quantity=0, stone_type=None, size=None):
    """Record a sales transaction with journal entry.

    Args:
        date: Transaction date
        customer_name: Customer name
        amount: Sale amount (for journal entry)
...
def record_purchase(date, vendor_name, amount, gst_amount, payment_type='cash', description="Purchase", quantity=0, stone_type=None, size=None):
    """Record a purchase with journal entry.

    Journal Entry:
        Cash Purchase:  Debit Inventory (if tracking stock) or Purchases, Credit Cash
        Credit Purchase: Debit Inventory (if tracking stock) or Purchases, Credit Accounts Payable

    Args:
        date: Transaction date
        vendor_name: Vendor/supplier name
        amount: Base purchase amount (before GST)
        gst_amount: GST amount (for ITC tracking)
        payment_type: 'cash' or 'credit'
        description: Additional description
        quantity: Quantity purchased (for COGS/inventory tracking)
    ...
    # Use Inventory account for inventory-tracking purchases, Purchases expense otherwise
    if item and item.rate_per_ton > 0:
        inventory_acc = get_or_create_account('Inventory', 'asset')
...
def record_salary_payment(date, employee_name, gross_salary, pf_deduction=0, employer_pf=0, description="Salary"):
    """Record salary payment with proper journal entry.
    
    FIXED: Now books gross salary expense + creates PF liability accounts.
    ...
    # First entry: Salary Expense -> Cash (net pay)
    if net_salary > 0:
        desc = f"Salary (Net) - {employee_name} - {description}"
        create_journal_entry(date, desc, salary_expense.id, cash_acc.id, net_salary)
    
    # Second entry: Salary Expense -> PF Payable (employee deduction)
    if pf_deduction > 0:
        desc = f"PF Deducted - {employee_name} - {description}"
        create_journal_entry(date, desc, salary_expense.id, pf_payable.id, pf_deduction)
    
    # Third entry: Employer PF Expense -> PF Payable (employer contribution)
    if employer_pf > 0:
        desc = f"Employer PF - {employee_name} - {description}"
        create_journal_entry(date, desc, pf_expense.id, pf_payable.id, employer_pf)
...
def record_payment(date, sale_id, amount, payment_mode='cash', notes='', description=""):
    ...
    create_journal_entry(date, desc, cash_acc.id, receivable_acc.id, amount)
